...
\copy src_sede FROM 'C:/temp/migracion-delicias/sede.csv' WITH (FORMAT csv, HEADER true, ENCODING 'WIN1252')
\copy src_producto FROM 'C:/temp/migracion-delicias/producto.csv' WITH (FORMAT csv, HEADER true, ENCODING 'WIN1252')
\copy src_venta FROM 'C:/temp/migracion-delicias/venta.csv' WITH (FORMAT csv, HEADER true, ENCODING 'WIN1252')
\copy src_detalle FROM 'C:/temp/migracion-delicias/detalle_venta.csv' WITH (FORMAT csv, HEADER true, ENCODING 'WIN1252')
...
SELECT
    ...
    v.total,
    CASE
        WHEN v.sede_id = 1 THEN 1
        WHEN v.sede_id = 2 THEN 7
        ELSE NULL
    END AS target_sede_id,
    v.modo_pago,
...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM venta_stage WHERE target_sede_id IS NULL) THEN
        RAISE EXCEPTION 'Existen ventas con sede origen sin mapeo hacia Steelsoft';
    END IF;
END $$;
...
SELECT
    ...
    i.sede_id AS target_sede_id,
    dp.codigo AS target_producto_id,
    dp.nombre AS target_nombre
...
LEFT JOIN producto_map pm
   ...
   AND pm.target_sede_id = vs.target_sede_id;
...
BEGIN
    IF EXISTS (
        ...
        RAISE EXCEPTION 'Existen detalles con producto sin mapeo hacia Steelsoft';
    END IF;
END $$;
...
SELECT
    ...
    ROW_NUMBER() OVER (
        ORDER BY vs.target_sede_id, vs.fecha, vs.source_venta_id
    ) AS rn
FROM venta_stage vs
LEFT JOIN (
    SELECT sede_id, MAX(numero_consecutivo) AS max_consecutivo
    FROM venta
    WHERE sede_id IN (1, 7)
    GROUP BY sede_id
) mx
...
SELECT target_sede_id AS sede_id, COUNT(*) AS ventas_por_sede
FROM venta_map
GROUP BY target_sede_id
ORDER BY target_sede_id;
